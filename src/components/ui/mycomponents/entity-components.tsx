...
import { Empty , EmptyContent , EmptyHeader , EmptyDescription, EmptyMedia,EmptyTitle,} from "../empty";
...
export const EntityContainer = ({
  ...
  return (
    <div className="p4 md:px-10 md:py-6 h-full">
...
export const EntitySearch = ({
  ...
  return (
    <div className="relative ml-auto">
      <SearchIcon className="size-3.5 absolute left-3 top-3.5 -translate-y-0.5 text-muted-foreground" />
      <Input
  ...
  return (
    <div className={cn("flex flex-col gap-y-4",  className)}>
        {items.map((item, index) => (
          <li key={getKey ? getKey(item, index) : index}>{renderItem(item, index)}</li>
        ))}
    </div>
  );
}
